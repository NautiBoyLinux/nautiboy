...
`$XDG_CONFIG_HOME/autostart/io.github.nautiboy.nautiboy.desktop`

When `XDG_CONFIG_HOME` is unset, the location is
`~/.config/autostart/io.github.nautiboy.nautiboy.desktop`. No system directory,
...
does not select media, send an LCD frame, or leave hardware mode.
